...
<xsl:stylesheet version="2.0"
    ...
    <xsl:template match="//ds:subsystem/ds:datasources/ds:datasource[@jndi-name='java:jboss/datasources/KeycloakDS']">
        <ds:datasource jndi-name="java:jboss/datasources/KeycloakDS" enabled="true" use-java-context="true" pool-name="KeycloakDS" use-ccm="true">
            <ds:connection-url>jdbc:jtds:sqlserver://${env.MSSQL_HOST}:${env.MSSQL_PORT:1433}/${env.MSSQL_DATABASE:keycloak};appname=KeyCloak</ds:connection-url>
            <ds:driver>sql_server</ds:driver>
            <ds:security>
    ...
    <xsl:template match="//ds:subsystem/ds:datasources/ds:drivers">
        <xsl:copy>
            <xsl:apply-templates select="node()|@*"/>
                <ds:driver name="sql_server" module="net.sourceforge.jtds">
                    <ds:xa-datasource-class>net.sourceforge.jtds.jdbcx.JtdsDataSource</ds:xa-datasource-class>
                </ds:driver>
        </xsl:copy>
    </xsl:template>
